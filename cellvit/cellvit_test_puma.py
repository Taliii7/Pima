...
def test_model(model, device, x_folder, output_folder, mag, iou):
	...
	for p in x_paths:
		...
		y_path = p.replace('.png', '_nuclei_masks.tiff')
...
def main():
	...
	x_folder = "../puma_data_cellvit"
	y_folder = "../puma_data_cellvit"
	output_folder_40x_0_5 = "../out_puma_0_5"
	output_folder_40x_0_6 = "../out_puma_0_6"

	checkpoint = "../CellViT-SAM-H-x40-AMP.pth"

	model, device = load_model(checkpoint)
	results, _, _ = test_model(model, device, x_folder, output_folder_40x_0_5, mag=40, iou=0.5)
	print(f"Results : {results}")

	results, _, _ = test_model(model, device, x_folder, output_folder_40x_0_6, mag=40, iou=0.6)
	print(f"Results : {results}")

	print("output_folder_40x_0_5/results.csv")
	evaluate_models.get_mean("../out_puma_0_5/results.csv")

	print("output_folder_40x_0_6/results.csv")
	evaluate_models.get_mean("../out_puma_0_6/results.csv")
